...
We can use subqueries in the FROM statement to create a temporary table to use in a query. Here we'll do it in the context of a join. What does the following do?

```
dbGetQuery(db, "select * from questions Q join questions_tags T on
               Q.questionid = T.questionid join
               (select max(viewcount) as maxview, tag from questions Q2
               join questions_tags T2 on Q2.questionid = T2.questionid
               group by tag) M on M.tag = T.tag
