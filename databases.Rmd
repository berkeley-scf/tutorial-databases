...
### 2.4.7) Complicated grouping queries
...
```
 ...
 - Compute cumulative calculations 
```{r, eval=TRUE}
...
(One can also choose a fixed number of rows by replacing 'range' with 'rows'.)
...
The ROWS and RANGE syntax allow one to specify the *window frame* in more flexible ways than simply the categories of a categorical variable.
...
Unfortunately I didn't set up a PostgreSQL version of the Stackoverflow database, but it would be interesting to see what query plan is involved in a join and when using indexes. 
