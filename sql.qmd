...
::: {.callout-warning title="`where` vs. `having`"}
To filter the result of a grouping operation, we need to use `having` rather than `where`. (`where` would filter before the application of the `group by`).
...
::: {.callout-warning title="Fields and `group by`"}
...
::: {.callout-tip title="`count` and NULL values"}
...
::: {.callout-warning title="Substrings"}
...
::: {.callout-tip title="Challenge (hard)"}
